""""
=========================================================================================================
Python script to demonstrates an artificial neural network
=========================================================================================================
This program implements a simple feed-foward neural network with one hidden layer,
trained for manual forward propagation, backward propagation, and gradient descent,
to learcn the XOR function.|
...
backpropagation.
"""
    return 1.0 / (1.0 + np.exp(-x))  # FIX 1: indented to be inside the function
...
class NeuralNetwork:
    ...
    def __init__(
        ...
        self.weights_input_hidden: np.ndarray = rng.uniform(
            -1.0, high=1.0, size=(self.input_size, self.hidden_size)
        )

        self.bias_hidden: np.ndarray = np.zeros((1, self.hidden_size))
        self.weights_hidden_output: np.ndarray = rng.uniform(
            -1.0, high=1.0, size=(self.hidden_size, self.output_size)
        )
        ...
        # backward propagation, so they are stored as attributes.
    ...
    def backward_propagation(  # FIX 2: indented to be inside the class
        ...
        """
        n_samples = inputs.shape[0]  # FIX 3: all method body indented to be inside the method
