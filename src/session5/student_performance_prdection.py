...
# Primary dataset path (preferred exact name). If not present, the loader
# will attempt to find any file matching 'student_scores*.csv' inside the
# repository-level `files/` folder (handles names like 'student_scores (1).csv').
DATA_FILE: Path = BASE_DIR / "files" / "student_scores.csv"
RESULTS_DIR: Path = BASE_DIR / "files" / "results"
RANDOM_STATE: int = 42
TEST_SIZE: float = 0.2
EPOCHS: int = 50
BATCH_SIZE: int = 32
...
def load_dataset(file_path: Path) -> pd.DataFrame:
    ...
    # If the exact path is not present, look for any nearby matching file
    if not file_path.exists():
        search_dir = BASE_DIR / "files"
        if search_dir.exists():
            matches = sorted(search_dir.glob("student_scores*.csv"))
            if matches:
                found = matches[0]
                print(f"[INFO] Using dataset file found at: {found}")
                file_path = found
            else:
                raise FileNotFoundError(f"Dataset file not found at: {file_path}")
        else:
            raise FileNotFoundError(f"Files directory not found at: {search_dir}")
...
def validate_dataset(dataset: pd.DataFrame) -> None:
    ...
    for cat_column in ["cat1_score", "cat2_score"]:
        if not dataset[cat_column].between(0,30).all():
            raise ValueError(f"{cat_column} values must be between 0 and 30.")
...
def plot_class_distribution(dataset: pd.DataFrame) -> None:
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(RESULTS_DIR / "student_performance_class_distribution.png")
        plt.close()
    except Exception as e:
...
def plot_training_history(history: tf.keras.callbacks.History) -> None:
    ...
    try:
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(RESULTS_DIR / "student_performance_training_history.png")
        plt.close()
    except Exception as e:
        raise RuntimeError(f"Failed to save training history plot: {e}") from e
...
def main() -> None:
    ...
    try:
        ...
        display_summary(len(x_train), len(x_test), accuracy)
...
# --------------------------------------------------------------------------------
# 13. Run the script by invoking it's main() function
# --------------------------------------------------------------------------------
